...
<div class="col-md-10">
				...
				<div class="panel panel-default">
					...
					<table class="table-bordered table table-hover">
						...
						<td><a href="javascript:;" onclick="edit({{$value->id}})" data-toggle="modal" data-target="#edit" class="glyphicon glyphicon-pencil"></a>&nbsp;&nbsp;&nbsp;<a href="javascript:;" onclick="deleted(this,{{$value->id}})" class="glyphicon glyphicon-trash"></a></td>
						</tr>
						@endforeach
		...
		function status(obj,id,status){
			...
			if(status){
				...
				$.post('/admin/admin/ajaxStatu',{id:id,"_token":"{{csrf_token()}}","status":"0"},function(data){
					if(data==1){
						$(obj).parent().html('<td><span class="btn btn-success" onclick="status(this,{{$value->id}},0)">正常</span></td>')
					}else{
						alert('修改失败');
					}
				})
				...
				$.post('/admin/admin/ajaxStatu',{id:id,"_token":"{{csrf_token()}}","status":"1"},function(data){
					if(data==1){
						$(obj).parent().html('<td><span class="btn btn-danger" onclick="status(this,{{$value->id}},1)">禁用</span></td>')
					}else{
						alert('修改失败');
					}
				})
			...
			$.post('/admin/admin',{str:str,'_token':'{{csrf_token()}}'},function(data){
				...
				if(data==1){
					...
					if(data.name){
						...
						str="<div class='alert alert-success'>正确</div>"
					}
					$("#userInfo").html(str);
					// 密码的提示信息
					if(data.pass){
